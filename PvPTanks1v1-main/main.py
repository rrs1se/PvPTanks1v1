...
class Player(GameSprite):
    ...
    def update_L(self):
        ...
        if keys[pygame.K_w] and self.rect.y > 0:
            x, y = self.calculate_new_xy((self.rect.x, self.rect.y), self.speed, self.angle)
            self.rect.center = round(x), round(y)
        if keys[pygame.K_s] and self.rect.y < monitor_height - self.height:
            x, y = self.calculate_new_xy((self.rect.x, self.rect.y), -self.speed, self.angle)
            self.rect.center = round(x), round(y)
        if keys[pygame.K_a]:
            self.angle -= 3
...
player1 = Player("tank1.png", 100, 400, 175, 100, 3, 90)
player2 = Player("tank2.png", 1300, 400, 175, 100, 3, 270)
